...
export function PopularLocations() {
  ...
  return (
    <section className="bg-gradient-to-b from-background via-card/30 to-background border-y border-border/60 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 py-20 md:px-8 lg:px-12 relative z-10">
        <div className="text-center space-y-6 mb-16">
          ...
          <h2
            className="text-4xl md:text-5xl font-serif font-bold bg-linear-to-r from-foreground to-primary bg-clip-text text-transparent"
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 md:gap-8 max-w-6xl mx-auto">
          {locations.map((location, index) => (
            <div
              key={location.name}
              className="group flex flex-col items-center justify-center p-8 bg-white/80 dark:bg-gray-800/80 border border-border/60 rounded-2xl hover:shadow-xl hover:shadow-primary/10 hover:border-primary/30 transition-all duration-500 hover:scale-105 backdrop-blur-sm hover:bg-white dark:hover:bg-gray-800 animate-fade-in"
            ...
            >
              ...
              <img
                ...
                alt={location.name}
                className="max-w-full h-12 object-contain opacity-70 group-hover:opacity-100 transition-all duration-300 group-hover:scale-110 mb-3"
              />
        ...
        <div className="text-center mt-16">
          <div className="bg-linear-to-r from-primary/5 to-primary/10 border border-primary/20 rounded-2xl p-8 max-w-2xl mx-auto backdrop-blur-sm">
            <h3 className="text-2xl font-bold text-foreground mb-4">Ready to Meet Someone Special?</h3>
            <p className="text-muted-foreground mb-6">
              Join thousands of singles who are finding real connections at places they already love.
            </p>
            <div className="flex items-center justify-center gap-2 text-primary font-medium">
              <MapPin className="w-5 h-5" />
              <span>Events happening near you right now</span>
            </div>
          </div>
        </div>
      </div>
